...
def setup_msolver(n, args, seed=None, restrict_map=None):
    ...
    try:
        ...
        msolver = msolverclass(n, bias=varbias, rand_seed=seed, dump=args.dump_map, restrict_map=restrict_map)
    except OSError as e:
        error_exit("Unable to load pyminisolvers library.", "Run 'make -C pyminisolvers' to compile the library.", e)
...
def setup_solvers(args, seed=None):
    ...
    msolver = setup_msolver(csolver.n, args, seed, restrict_map=csolver.restrict_map)
...
def print_result(result, args, stats, num_constraints):
    ...
    output = result[0]
    output = "%s (of size %d)" % (output, len(result[1]))
    if args.alltimes:
