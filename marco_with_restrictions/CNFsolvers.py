    ...
    def parse_dimacs(self, f):
        ...
        for line in f:
            ...
            if line.startswith(b'c'):
                tokens = line.split()
                group_restriction_clause_in = (len(tokens) > 1) and (tokens[1] == b"restrict")

                if gcnf_in and group_restriction_clause_in:
                    # lines describing group restrictions look as follows:
                    # c restrict -1 2 3 0
                    assert self.n > 2
                    vals = line.split()
                    assert vals[-1] == b'0'
                    group_restriction_clause = [int(x) for x in vals[2:-1]]
                    #print(group_restriction_clause)
                    self.restrict_map.append(group_restriction_clause)
                    #print(self.restrict_map)
                continue
...
class MUSerSubsetSolver(MinisatSubsetSolver):
    ...
    def shrink(self, seed):
        ...
        # NEW: this checks is also not valid when there are restrictions on groups
